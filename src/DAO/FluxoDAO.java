...
public class FluxoDAO {
    ...
    PreparedStatement stmt = null;
        ...
        try {
            stmt = con.prepareStatement("INSERT INTO erp_beer.fluxo (Caixa,Loja,Operador,Abertura_data,Fechamento_data,Fluxo_de_caixa) VALUES(?,?,?,?,?,?)");
    ...
    public List<Fluxo> readfluxo(){
        ...
        try {
            stmt = con.prepareStatement("SELECT * FROM erp_beer.fluxo ORDER BY Fechamento_data");
     ...
     public List<Fluxo> Especifico(String loja, String caixa, java.sql.Date data){
        ...
        try {
            stmt = con.prepareStatement("SELECT * FROM erp_beer.fluxo WHERE Loja LIKE ? AND Caixa = ? AND Abertura_data = ?");
     ...
     public float FluxoSoma(String loja, java.sql.Date data1, java.sql.Date data2){
        ...
        try {
            stmt = con.prepareStatement("SELECT * FROM erp_beer.fluxo WHERE Loja LIKE ? AND Abertura_data between ? and ?");
            stmt.setString(1, loja);
     ...
     public List<Fluxo> EspecificoLoja(String loja){
        ...
        try {
            stmt = con.prepareStatement("SELECT * FROM erp_beer.fluxo WHERE Loja LIKE ?");
            stmt.setString(1, loja);
            rs = stmt.executeQuery();
     ...
     public float FluxoSomaTotal(java.sql.Date data1, java.sql.Date data2){
        ...
        try {
            stmt = con.prepareStatement("SELECT * FROM erp_beer.fluxo WHERE Abertura_data between ? and ?");
